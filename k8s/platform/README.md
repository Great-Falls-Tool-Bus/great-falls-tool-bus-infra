...
| `members-greatfallstoolbus-org-production/networkpolicy.yaml` | default-deny **both directions** + named allows | ingress: cloudflared→web:3000, prometheus; egress: DNS + member-db:5432 only; Stripe behavior is landed, but provider egress remains closed until a separate test-mode runtime activation review |
