...
2. Before proceeding, require PR #118 to land as the sole names-only
   SecretContract for the exact namespace/name target. Then require the
   governed consumer-enrollment sole controller (TIN-3768 / TIN-2609) to
   project the ghcr-pull bytes into that target and require the resulting
   kubernetes.io/dockerconfigjson Secret to be observed by exact name/type/key.
   The controller is not currently live, so absence is a real STOP. No
   out-of-band kubectl, per-tenant SOPS, or operator Secret creation is an
   admitted substitute. Also require the attended database path to prove Ready,
