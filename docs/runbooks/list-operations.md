...
REST API 3.1) in namespace `latoolb-us-production`, context `honey`. TIN-4268
records the later live `subscription_policy=confirm` observation for
`discuss@`; the ratified target is now `moderate`.
...
| `discuss@latoolb.us` | Public community board | `public` | `moderate` (member-provisioned) | `hold` | `true` |
...
- **`discuss@latoolb.us`** is open to read, not to self-subscribe. The archive
  is **public**; only Active-member provisioning may add writers, and
  non-member posts are **held** for moderation rather than accepted.
...
- The ratified baseline in section 5 is product truth, not an apply carrier.
  TIN-4268 remains open until the real exact-main owner transaction converges
  the live setting and records readback. Do not add an inert manifest or use a
  manual PATCH/dispatch as a fallback. The roster reconciler in section 8 is a
  separate add-only projection.
...
On either list (`moderate` target), a candidate who mails a `-join@` address or
a REST add without `pre_approved` lands in the requests queue. On `discuss@`,
discard or reject non-member self-subscription requests; Active-member
provisioning is the only writer-admission path. List pending requests and act
with `accept`, `discard`, `reject`, or `defer`:
...
> This operator procedure is not the TIN-4268 convergence path. Do not use it
> to paper over the live `discuss@` divergence; that change must arrive through
> the protected exact-main owner transaction with readback.
...
These are the ratified targets. The 2026-07-04 baseline was confirmed live that
day; the `discuss@` subscription target was amended to `moderate` after the
later `confirm` observation recorded on TIN-4268.
...
The same setting on `discuss@` preserves public anonymous reading while making
Active-member provisioning the only writer-admission path. Until the owner
transaction proves convergence, `confirm` is observed drift, not a restore
target.
