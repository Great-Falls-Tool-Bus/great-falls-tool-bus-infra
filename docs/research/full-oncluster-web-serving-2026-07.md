...
| Cluster kubeconfig | **NEVER** | namespace-scoped workload kubeconfig + `ARC_RUNNERS_KUBECONFIG_B64` in protected envs | mints the namespace-scoped SA (`deploy/tenants/great-falls-tool-bus/rbac.yaml` → `latoolb-us-production`, TIN-2382) | overlay Actions secrets |
...
- `great-falls-tool-bus-infra` (this overlay): `.sops.yaml`, `secrets/README.md`,
  ...
  and the legacy ARC continuity declarations in `config/organization.yaml`
  and `tofu/stacks/arc-runners/great-falls-tool-bus.tfvars` (not v4
