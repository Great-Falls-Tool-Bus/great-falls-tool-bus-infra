...
# TIN-4227 third one-use bridge (operator instruction, 2026-08-31 06:30 ET;
# decisions/0022 Amendment 3, PENDING -- not yet ratified in meta). Same shape
...
# its terminal receipt. This is a SCAFFOLD: WEB_GENERATION43_BRIDGE_SHA256 and
...
# WEB_GENERATION43_BRIDGE_SHA256 is the sha256 of the exact workflow bytes;
...
WEB_GENERATION43_TARGET_SOURCE = "0c92ec1ce8b6c42eb17488008c28f487fe01b939"
WEB_GENERATION43_TARGET_IMAGE = "ghcr.io/great-falls-tool-bus/gftb-site@sha256:3096c166a25300581e89101effa078dcf4100c638b0ce357f473f485624d6862"
WEB_GENERATION43_DEPLOYMENT = Path("k8s/web/greatfallstoolbus-org-production/deployment.yaml")
...
# recomputed once the generation-43 TARGET_SOURCE_SHA/TARGET_IMAGE pin merges
...
# scan_web_generation43_bridge_contract compare the two.
WEB_GENERATION43_INLINE_FREEZE = re.compile(
    r"sha256sum --check --strict <<'HASHES'\n(.*?)\n *HASHES\n", re.DOTALL
)
WEB_GENERATION43_FROZEN_INPUT_SHA256 = {
    ...
    Path("k8s/web/greatfallstoolbus-org-production/deployment.yaml"): "47ec6f53b9804e5dfe271fe8bb20f6ebc0ea4246e5935f0fbf8e8b15d972457e",
...
HOSTED_WORKFLOW_JUST_ALLOWLIST = {
    ...
    "web-stack-render",
    "web-stack-validate",
    # Exact, byte-pinned TIN-4227 third bridge only. scan_workflows subtracts
...
def scan_web_generation43_bridge_contract(
    workflow_text: str, deployment_text: str
) -> list[Finding]:
    findings: list[Finding] = []
    observed_digest = hashlib.sha256(workflow_text.encode("utf-8")).hexdigest()
    if observed_digest != WEB_GENERATION43_BRIDGE_SHA256:
        findings.append(
            Finding(
                "web-generation43-bridge-bytes",
                WEB_GENERATION43_BRIDGE_WORKFLOW,
                1,
                "The third one-time parity bridge changed outside its exact reviewed receipt; replace it only through the receiver-first GF-I09 cutover.",
            )
        )
    ...
    if image_values != [WEB_GENERATION43_TARGET_IMAGE]:
        findings.append(
            Finding(
                "web-generation43-desired-state-freeze",
                WEB_GENERATION43_DEPLOYMENT,
                1,
                "Until GF-I09 replaces the bridge, the web desired state must remain the exact generation-43 target; unrelated infra changes may proceed.",
            )
        )
    for path, expected_digest in WEB_GENERATION43_FROZEN_INPUT_SHA256.items():
        candidate = REPO / path
        observed_digest = hashlib.sha256(candidate.read_bytes()).hexdigest() if candidate.is_file() else "missing"
        if observed_digest != expected_digest:
            findings.append(Finding("web-generation43-render-input-freeze", path, 1, "The third one-time bridge admits only the exact reviewed render and recipe inputs until receiver-first GF-I09 cutover."))
    inline_match = WEB_GENERATION43_INLINE_FREEZE.search(workflow_text)
    ...
    if inline_freeze != WEB_GENERATION43_FROZEN_INPUT_SHA256:
        findings.append(
            Finding(
                "web-generation43-inline-freeze-agreement",
                WEB_GENERATION43_BRIDGE_WORKFLOW,
                1,
...
def scan_workflows() -> list[Finding]:
    ...
    gen43_bridge_path = REPO / WEB_GENERATION43_BRIDGE_WORKFLOW
    gen43_deployment_path = REPO / WEB_GENERATION43_DEPLOYMENT
    if not gen43_bridge_path.is_file() or not gen43_deployment_path.is_file():
        findings.append(
            Finding(
                "web-generation43-bridge-missing",
                WEB_GENERATION43_BRIDGE_WORKFLOW,
                1,
                "The third one-time parity bridge and its frozen desired-state carrier must remain together until the receiver-first GF-I09 cutover.",
            )
        )
    else:
        findings.extend(
            scan_web_generation43_bridge_contract(
    ...
    for rel in sorted(workflow_paths):
        ...
        if rel == WEB_GENERATION43_BRIDGE_WORKFLOW:
            scoped_forbidden = forbidden_recipes - set(WEB_GENERATION43_BRIDGE_RECIPES)
        findings.extend(
            scan_workflow_text(
                workflow_text,
                rel,
...
def self_test() -> None:
    ...
    # NOTE: the four checks below require WEB_GENERATION43_BRIDGE_SHA256,
    # WEB_GENERATION43_TARGET_SOURCE, and WEB_GENERATION43_TARGET_IMAGE to
    ...
    # run.
    gen43_bridge_text = (REPO / WEB_GENERATION43_BRIDGE_WORKFLOW).read_text(encoding="utf-8")
    gen43_deployment_text = (REPO / WEB_GENERATION43_DEPLOYMENT).read_text(encoding="utf-8")
    if scan_web_generation43_bridge_contract(gen43_bridge_text, gen43_deployment_text):
        raise SystemExit("self-test FAILED: committed generation-43 bridge contract drifted")
    mutated_gen43_bridge = gen43_bridge_text.replace(WEB_GENERATION43_TARGET_SOURCE, "0" * 40, 1)
    if not any(
        finding.rule == "web-generation43-bridge-bytes"
        for finding in scan_web_generation43_bridge_contract(mutated_gen43_bridge, gen43_deployment_text)
    ):
        raise SystemExit("self-test FAILED: generation-43 bridge mutation was accepted")
    mutated_gen43_deployment = gen43_deployment_text.replace(WEB_GENERATION43_TARGET_IMAGE, "ghcr.io/great-falls-tool-bus/gftb-site@sha256:" + "0" * 64, 1)
    if not any(
        finding.rule == "web-generation43-desired-state-freeze"
        for finding in scan_web_generation43_bridge_contract(gen43_bridge_text, mutated_gen43_deployment)
    ):
        raise SystemExit("self-test FAILED: generation-43 desired-state drift was accepted")
    for stale_digest in sorted(WEB_GENERATION43_FROZEN_INPUT_SHA256.values()):
        stale_inline = gen43_bridge_text.replace(stale_digest, "0" * 64, 1)
        if not any(
            finding.rule == "web-generation43-inline-freeze-agreement"
            for finding in scan_web_generation43_bridge_contract(stale_inline, gen43_deployment_text)
        ):
            raise SystemExit("self-test FAILED: a stale inline render-input freeze digest was accepted")
    dropped_inline = WEB_GENERATION43_INLINE_FREEZE.sub("sha256sum --check --strict <<'HASHES'\nHASHES\n", gen43_bridge_text, count=1)
    if not any(
        finding.rule == "web-generation43-inline-freeze-agreement"
        for finding in scan_web_generation43_bridge_contract(dropped_inline, gen43_deployment_text)
    ):
