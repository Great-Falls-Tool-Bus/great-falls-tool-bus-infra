...
This file is the names-only inventory of what actually lives (or will live)
in this overlay. No values, hostnames, or key material in this README — ever.
...
| `cloudflare-api-token-gftb-pages` | **Unused continuity HOLD.** This historical name may still exist in operator or repository custody, but the Pages project and publisher are retired. Do not recreate, copy, exercise, rotate, or revoke it without explicit per-item operator direction. | None; no GFTB web deployment consumer |
| `cf-account-id` | House Cloudflare account identifier retained in operator custody; it is not an edge-stack input and is not a site publication credential. | None in the GFTB web deployment path |
...
| `web-apply-kubeconfig` | Namespace-scoped credential for the current attended web release transaction. It cannot create namespaces and is retained only until the shared v4 owner-overlay path proves takeover. | `just web-release-server-dry-run`, `just web-release-apply`, and read-only web health/drift recipes |
| `google-sso-client-id` | OAuth 2.0 client id of the Google Cloud "Web application" credential backing the live Google Workspace CF Access IdP (`cf-access-google-sso` runbook). Same value as the protected `edge` environment secret `GOOGLE_SSO_CLIENT_ID`. Not sensitive on its own but paired with the secret below; kept out of git. Required for plan/apply/drift whenever `ENABLE_GOOGLE_SSO=true`; workflows check presence only and fail closed if it is absent. | `just edge-zones-plan` / `just edge-zones-apply` / scheduled edge drift (`TF_VAR_google_sso_client_id`) |
